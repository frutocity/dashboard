...
const ExclusiveOffersTable = ({ list, type }) => {
  ...
  const handleDelete = (id) => () => {
    ...
    if (type == "best-selling") {
      ...
      API.removebestSellingList({ params: { _id: id } }, (d) => {
        API.bestSellingList({})
      ...
      API.removemostPopularList({ params: { _id: id } }, (d) => {
        API.mostPopularList({})


      })
    } else if (type == "exclusive-offer") {


      API.removeExclusiveOfferList({ params: { _id: id } }, (d) => {
...
export default ExclusiveOffersTable
