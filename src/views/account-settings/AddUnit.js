...
import { BannerDataAtom, bannerModalAtom, detailsAtom, detailsCountAtom, UnitDataAtom } from 'src/helpers/jotai/atom'
...
const AddUnit = () => {
  ...
  const updateBanner = () => {
    console.log({ _id: banner._id, name: values.name })
    API.updateUnit({ body: { _id: banner._id, name: values.name } }, (d) => {
      ...
      <form>
        <Grid container spacing={7}>
          ...
          <Grid item xs={12} sm={6}>
            <TextField fullWidth label='Type' value={values.name} placeholder='' onChange={handleChange("name")} />
          </Grid>
