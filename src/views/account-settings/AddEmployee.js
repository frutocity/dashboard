...
import { BannerDataAtom, bannerModalAtom, detailsAtom, detailsCountAtom, employeeModalAtom, exchangeDataAtom, exchangeModalAtom, storeDataAtom, storeModalAtom } from 'src/helpers/jotai/atom'
import toast from 'react-hot-toast'
import { Switch } from '@mui/material'
import { roleListAtom, storeListAtom } from 'src/helpers/jotai/apiAtoms'
...
const AddEmployee = () => {
  ...
  const [roleList, setRoleList] = useAtom(roleListAtom)
        ...
        <Grid container spacing={7}>
          ...
          <Grid item xs={12} sm={6}>
            <FormControl fullWidth>
              <InputLabel>{storeList.length ? "Store" : "No Store added in list"}</InputLabel>
              {
                storeList.length ?
          ...
          <Grid item xs={12} sm={6}>
            <FormControl fullWidth>
              <InputLabel>{roleList.length ? "Role" : "No Role added in list"}</InputLabel>
              {
                roleList.length ?
                  <Select label='Role' onChange={handleChange("role")} value={values.role} >
                    {
                      roleList.map(c => {
                        return (<MenuItem value={c?._id ?? ""}>{c?.name ?? ""}</MenuItem>)
                      })
